...
struct MagicComments {
    test_kind: TestKind,
    /// The options with which to run obol.
    obol_opts: Vec<String>,
...
static HELP_STRING: &str = unindent!(
    ...
    - `//@ obol-arg=<single charon cli option>`
...
fn parse_magic_comments(input_path: &std::path::Path) -> anyhow::Result<MagicComments> {
    ...
    for line in read_to_string(input_path)?.lines() {
        ...
        } else if let Some(charon_opt) = line.strip_prefix("obol-arg=") {
            comments.obol_opts.push(charon_opt.to_string());
...
fn perform_test(test_case: &Case) -> anyhow::Result<()> {
    ...
    // Run Charon.
    let mut cmd = Command::cargo_bin("obol")?;
    cmd.arg("rustc");
    ...
    if matches!(
        ...
        cmd.arg("--dest-file");
        cmd.arg(test_case.input_path.with_extension("postcard"));
    }
    cmd.args(&test_case.magic_comments.obol_opts);
